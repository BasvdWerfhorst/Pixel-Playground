...
function check1pair(){
    ...
    for (let kaartenA of kaarten[1].concat(kaarten[2])) {
        endHandA.push(kaartenA.split("_of_")[0]);
    }
    for (let kaartenS of kaarten[0].concat(kaarten[2])) {
        endHandS.push(kaartenS.split("_of_")[0]);
    }
    for(let eha of endHandS){
        times = endHandS.filter(x => x === eha);
        if(times.length === 2){
            pairS = eha;
        }
    }
    ...
    for(let eha of endHandA){
        times = endHandA.filter(x => x === eha);
        if(times.length === 2){
            pairA = eha;
        }
    } 
    ...
    console.log(endHandA);
    console.log(endHandS);
}
function highKart(){
    for (let kaartenA of kaarten[1].concat(kaarten[2])) {
        endHandA.push(kaartenA.split("_of_")[0]);
    ...
    let aiCartPoints = endHandA.map(cartpoint)
}
function cartpoint(kaart){
